...
int main()
{
		...
		&IMU_Read[18],&IMU_Read[19],&IMU_Read[20],&IMU_Read[21])) != EOF)
	{
		...
		if (IMU_Count % 100 == 0)  //1s
		{
			...
			matrix* Cbn = matrix_calloc(3,3);
			for (int ii=0; ii<3; ii++)
			for (int jj=0; jj<3; jj++)
			matrix_set(Cbn, ii,jj, SystemData.Cb_n[ii][jj]);
			matrix_set(Vb, 0,0, 0);
			matrix_set(Vb, 1,0, IMU_Read[6]);
			matrix_set(Vb, 2,0, 0);
			matrix_mul(Cbn, Vb, Vn);
			for (int jj=0; jj<3; jj++)
			matrix_set(Vb, jj,0, OutputData.Vg[jj]);
			matrix_sub(Vb, Vn);
			matrix_set(kfData.Yk, 0,0, matrix_get(Vb,0,0));
			matrix_set(kfData.Yk, 1,0, matrix_get(Vb,1,0));
			matrix_set(kfData.Yk, 2,0, matrix_get(Vb,2,0));
			matrix_free(Vb);
			matrix_free(Vn);
			matrix_free(Cbn);
