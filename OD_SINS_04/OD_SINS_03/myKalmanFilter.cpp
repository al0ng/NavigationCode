...
void InitKalmanFilter()
{
	...
	kfData.ts = ins.nnts;				//应当由ins结构体中的nnts确实
...
void KalmanFilterSetPhi(double *imu_avp)
{
	...
	matrix *qnb = matmas_calloc(4,1,mas);
	matrix *Cnb = matmas_calloc(3,3,mas);
	matrix *fb  = matmas_calloc(3,1,mas);
	matrix *fg  = matmas_calloc(3,1,mas);
	extern EARTH earth;
	extern insdata ins;	//惯导解算数据结构体
	int i;
	for (i=0; i<6; i++)
		imu[i] = imu_avp[i];
	for(i=0; i<9; i++)
