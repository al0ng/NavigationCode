...
void InitKalmanFilter()
{
	...
	matrix_set(kfData.Pk, 9,9,   pow(5.0*Deg_Rad, 2));
	matrix_set(kfData.Pk, 10,10, pow(5.0*Deg_Rad, 2));
	matrix_set(kfData.Pk, 11,11, pow(5.0*Deg_Rad, 2));
	...
	matrix_set(kfData.Rk, 0,0, pow(1.0, 2));
	matrix_set(kfData.Rk, 1,1, pow(1.0, 2));
	matrix_set(kfData.Rk, 2,2, pow(1.0, 2));
