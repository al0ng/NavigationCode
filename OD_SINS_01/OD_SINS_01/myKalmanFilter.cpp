...
void InitKalmanFilter()
{
	...
	matrix_set(kfData.Pk, 9,9,   pow(150*Dph_Rps, 2));
	matrix_set(kfData.Pk, 10,10, pow(150*Dph_Rps, 2));
	matrix_set(kfData.Pk, 11,11, pow(150*Dph_Rps, 2));
	matrix_set(kfData.Pk, 12,12, pow(1*mg, 2));
	matrix_set(kfData.Pk, 13,13, pow(1*mg, 2));
	matrix_set(kfData.Pk, 14,14, pow(1*mg, 2));
...
// 	matrix_set(kfData.Qk, 5,5, pow(0.001*mg, 2));
	matrix_set(kfData.Qk, 0,0, pow(1.0*dp05h, 2));
	matrix_set(kfData.Qk, 1,1, pow(1.0*dp05h, 2));
	matrix_set(kfData.Qk, 2,2, pow(1.0*dp05h, 2));
...
void KalmanFilterOut(FILE *fp_KFRes)
{
	...
	for (int j=0; j<3; j++)
		fprintf(fp_KFRes, "%.10lf ", matrix_get(kfData.Yk, j,0));
	fprintf(fp_KFRes, "%4d\n", i);
// 	for (int i=0; i<15; i++)
// 	{
// 		for (int j=0; j<15;j++)
